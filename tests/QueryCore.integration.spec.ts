import { QueryCore, QueryCoreOptions, EndpointOptions, EndpointState } from '../src/QueryCore';
...
describe('QueryCore - Integration Tests', () => {
  ...
  it('Lifecycle: fetch, cache (LS), subscribe, retrieve, then manual refetch', async (done) => {
    ...
    qc.subscribe(endpointKey, (state) => {
      ...
      if (callCount === 1) { // Initial state (empty as cache is clear)
        expect(state.data).toBe(undefined);
        expect(state.isLoading).toBe(false); // defineEndpoint loads from cache (miss)
      } else if (callCount === 2 && state.isLoading) { // Loading (from auto-fetch on subscribe due to no data)
         expect(state.data).toBe(undefined);
      } else if (callCount === 3 && !state.isLoading && !state.isError) { // Fetched initialUser
      ...
      } else if (callCount === 4 && state.isLoading) { // Loading for updatedUser
         expect(state.data).toEqual(initialUser); // Previous data still there
      } else if (callCount === 5 && !state.isLoading && !state.isError) { // Fetched updatedUser
  ...
  it('Automatic refetching with refetchAfter and multiple subscribers', async (done) => {
    ...
    qc.subscribe(`${endpointKey}_sub1`, (state) => { // Using different key to avoid clash if needed by test runner
        sub1Notifications++;
        if(state.data?.name === updatedUser.name) sub1FinalData = state.data;
    });

    qc.subscribe(`${endpointKey}_sub2`, (state) => { // Using different key
        sub2Notifications++;
    ...
    qc.subscribe(endpointKey, (state) => {
        ...
        if (state.data?.name === initialUser.name && primarySubNotifications > 2) { // After initial load from refetch
    ...
    let s1Loading = false, s2Loading = false;
    let s1UpdateCount = 0, s2UpdateCount = 0;
  ...
  it('Correct state transitions with IndexedDB', async (done) => {
    ...
    qc.subscribe(endpointKey, (state) => {
        ...
        if (states.length === 5) { // initial(undef), loading(undef), success(initialUser), loading(initialUser), success(updatedUser)
            expect(states[0].isLoading).toBe(false); expect(states[0].data).toBeUndefined(); // initial after define
            expect(states[1].isLoading).toBe(true); expect(states[1].data).toBeUndefined(); // loading for initialUser
            expect(states[2].isLoading).toBe(false); expect(states[2].data).toEqual(initialUser); // success initialUser
            expect(states[3].isLoading).toBe(true); expect(states[3].data).toEqual(initialUser);  // loading for updatedUser
            expect(states[4].isLoading).toBe(false); expect(states[4].data).toEqual(updatedUser); // success updatedUser
            done();
        }
    });
